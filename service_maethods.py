...
async def contains_stopphrase(app, msg) -> bool:
    ...
    logger.info("\033[93mлогер 1 старт\033[0m")
    for stop in stopwords:
        logger.info("\033[93mзашли в цикл проверки слов\033[0m")  # жёлтый
        if stop and matcher(msg, stop):
            logger.info(f"\033[92mСообщение проигнорировано по стоп-фразе: {stop}\033[0m")  # зелёный
            # await app.send_message(TARGET_CHAT, f"Сообщение проигнорировано по стоп-фразе: {stop}")
            logger.info("\033[92mотправил в чат сообщение\033[0m")  # зелёный
            found_stopword = True
            return found_stopword
    logger.info("\033[94mне может слово найти\033[0m")
    return found_stopword;
